...
X.test <- strsplit(gsub("  +", " ", gsub("^ +", "", readLines("test/X_test.txt"))), " ")
...
X.train <- strsplit(gsub("  +", " ", gsub("^ +", "", readLines("train/X_train.txt"))), " ")
...
activity <- read.table("activity_labels.txt", stringsAsFactors = FALSE)$V2
